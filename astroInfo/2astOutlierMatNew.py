...
# in getAllObsratings
#@profile
def plotAstRatings( astData ):
    ratings, astRating = getAstRating( astData )

    # TODO: write ploting stuff here
...
def fillSigmaMatrix( name, asteroid, sigmaMatrix, fltr, outFlag, plot ):
    ...
    if fltrType == 1:
        ...
        rowAttrs = attrData
        ratings, astRating = getAstRating( asteroid )
...
########################################################################################
def viewOne( astArgs, exportFlg, exportArgs, plots ):
...
def main( ):
    ...
    if maxIn == 1:
        viewOne( astArgs, featFltr, exportFlg, exportArgs, plots )
    else:
